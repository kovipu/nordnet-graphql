...
export class NordnetAccountInfo {
  @Field(type => ID, { nullable: true, description: "The account identifier (id) of the account. The id is unique within the session. Not applicable for partners" })
  accid?: number;
  ...
  @Field({ description: "account_sum + full_marketvalue + interest + forward_sum + future_sum + unrealized_future_profit_loss" })
  own_capital: Amount;
...
export class NordnetInstrument {
  ...
  @Field({ nullable: true, description: "The instrument group. Wider description than instrument type. The description is available in the instrument type lookup" })
  instrument_group_type?: string;

  @Field(type => ID, { description: "Unique identifier of the instrument. Can in some cases be 0 if the instrument is not tradable" })
  ...
  @Field(type => Int, { nullable: true, description: "The mifid2 category of an instrument. Used to determine if a user can trade the instrument" })
  ...
  @Field({ nullable: true, description: "Price type when trading, not available for all markets. Example: monetary_amount, percentage, yield" })
  price_type?: string;
  ...
  // @Field(type => [NordnetTradable], { nullable: true, description: "The tradables that belongs to the instrument. If the instrument is not tradable this field is left out" })
...
export class NordnetPosition {
  @Field(type => Int, { nullable: true, description: "The account identifier (id) of the account. The id is unique within the session. Not applicable for partners" })
  accid?: number;
...
export class NordnetAccount {
  ...
  @Field({ nullable: true, description: "Description to why the account is blocked. The language specified in the request is used in this reply so it can be displayed to the end user" })
  ...
  @Field({ nullable: true, description: "Shareville alias that is connected to this account number. Only available for certain systems" })
  shareville_alias?: string;

  @Field({ description: "Translated account type." })
  type: string;

  @Field(type => [NordnetAccountInfo], { description: "The account info summary gives details of one or more account." })
  info: NordnetAccountInfo[];

  @Field(type => NordnetLedgerInfo, { nullable: true, description: "Information about the currency ledgers of an account" })
